  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(20).font('Helvetica-Bold')
         .fillColor('#000000')
         .text('INVESTMENT APPLICATION FORM', { align: 'center' });
      ...
      const col1 = 180, col2 = 160, col3 = 160;
      ...
      // Left column
      doc.fontSize(10).font('Helvetica-Bold')
         .text('Account Start:', leftCol, currentY);
      doc.font('Helvetica')
         .text(formatDatePDF(case_data.saving_account_start_date), leftCol + 80, currentY);

      doc.font('Helvetica-Bold')
         .text('Deposit Type:', leftCol, currentY + lineHeight);
      doc.font('Helvetica')
         .text(case_data.deposit_type || '', leftCol + 80, currentY + lineHeight);

      doc.font('Helvetica-Bold')
         .text('Duration:', leftCol, currentY + lineHeight * 2);
      doc.font('Helvetica')
         .text((case_data.deposit_duration_years || '') + ' years', leftCol + 80, currentY + lineHeight * 2);

      doc.font('Helvetica-Bold')
         .text('FD Amount:', leftCol, currentY + lineHeight * 3);
      doc.font('Helvetica')
         .text(formatCurrencyPDF(case_data.fixed_deposit_total_amount), leftCol + 80, currentY + lineHeight * 3);

      doc.font('Helvetica-Bold')
         .text('RD Amount:', leftCol, currentY + lineHeight * 4);
      doc.font('Helvetica')
         .text(formatCurrencyPDF(case_data.recurring_deposit_total_amount), leftCol + 80, currentY + lineHeight * 4);

      // Right column
      doc.font('Helvetica-Bold')
         .text('FD Rate:', rightCol, currentY);
      doc.font('Helvetica')
         .text(formatPercentagePDF(case_data.interest_rate_fd), rightCol + 80, currentY);

      doc.font('Helvetica-Bold')
         .text('RD Rate:', rightCol, currentY + lineHeight);
      doc.font('Helvetica')
         .text(formatPercentagePDF(case_data.interest_rate_recurring), rightCol + 80, currentY + lineHeight);

      doc.font('Helvetica-Bold')
         .text('Savings Amount:', rightCol, currentY + lineHeight * 2);
      doc.font('Helvetica')
         .text(formatCurrencyPDF(case_data.saving_account_total_amount), rightCol + 80, currentY + lineHeight * 2);

      doc.font('Helvetica-Bold')
         .text('Investment Amount:', rightCol, currentY + lineHeight * 3);
      doc.font('Helvetica')
         .text(formatCurrencyPDF(case_data.dnyanrudha_investment_total_amount), rightCol + 80, currentY + lineHeight * 3);

      doc.font('Helvetica-Bold')
         .text('Savings Rate:', rightCol, currentY + lineHeight * 4);
      doc.font('Helvetica')
         .text(formatPercentagePDF(case_data.interest_rate_saving), rightCol + 80, currentY + lineHeight * 4);

      doc.font('Helvetica-Bold')
         .text('Dynadhara Rate:', rightCol, currentY + lineHeight * 5);
      doc.font('Helvetica')
         .text(formatPercentagePDF(case_data.dynadhara_rate), rightCol + 80, currentY + lineHeight * 5);
      ...
      // Declaration Section
      doc.fontSize(10).font('Helvetica')
         .text('I hereby solemnly affirm that the information provided above is true and correct to the best of my knowledge and belief, and nothing material has been concealed therefrom. I understand that any false information may lead to rejection of my application.', {
           align: 'justify',
           lineGap: 4
         });
      ...
      doc.fontSize(10).font('Helvetica-Bold')
         .text('✓ VERIFIED AND CONFIRMED BY APPLICANT', { align: 'center' });

      doc.moveDown(1.5);

      // System Generated Text
      doc.fontSize(9).font('Helvetica-Bold')
         .text('System Generated Document', { align: 'center' });
      
      doc.fontSize(8).font('Helvetica')
         .text('No manual signature required', { align: 'center' });
      ...
      doc.fontSize(9).font('Helvetica')
         .text('Place: ______', 50, signatureY);
      doc.text('Date: ___/___/______', 300, signatureY);

      doc.moveDown(2);

      doc.fontSize(9).font('Helvetica-Bold')
         .text('Signature of Applicant', 50, doc.y);
      doc.fontSize(8).font('Helvetica')
      ...
      const caseTable = {
        ...
        datas: [
          { field: 'Saving Account Start Date', details: case_data.saving_account_start_date ? new Date(case_data.saving_account_start_date).toLocaleDateString() : 'N/A' },
          ...
          { field: 'Fixed Deposit Amount', details: case_data.fixed_deposit_total_amount ? `₹${case_data.fixed_deposit_total_amount}` : 'N/A' },
          { field: 'FD Interest Rate', details: case_data.interest_rate_fd ? `${case_data.interest_rate_fd}%` : 'N/A' },
          { field: 'Saving Account Amount', details: case_data.saving_account_total_amount ? `₹${case_data.saving_account_total_amount}` : 'N/A' },
          { field: 'Saving Interest Rate', details: case_data.interest_rate_saving ? `${case_data.interest_rate_saving}%` : 'N/A' },
          { field: 'Recurring Deposit Amount', details: case_data.recurring_deposit_total_amount ? `₹${case_data.recurring_deposit_total_amount}` : 'N/A' },
          { field: 'RD Interest Rate', details: case_data.interest_rate_recurring ? `${case_data.interest_rate_recurring}%` : 'N/A' },
          { field: 'Dnyanrudha Investment', details: case_data.dnyanrudha_investment_total_amount ? `₹${case_data.dnyanrudha_investment_total_amount}` : 'N/A' },
      ...
      if (documents && Object.keys(documents).length > 0) {
        ...
        Object.entries(documents).forEach(([exhibitName, exhibitDocs], index) => {
          ...
          if (Array.isArray(exhibitDocs) && exhibitDocs.length > 0) {
            ...
            doc.fontSize(10).font('Helvetica')
               .fillColor('#7f8c8d')
               .text('No documents available for this exhibit');
          }
        });
